...
sub canDoAction {
    ...
    if ( $action eq 'pause' ) {
        Plugins::BBCSounds::ActivityManagement::heartBeat(
            Plugins::BBCSounds::ProtocolHandler->getId($url),
            Plugins::BBCSounds::ProtocolHandler->getPid($url),
            'paused',
            floor(
                $client->playingSong()->master->controller->playingSongElapsed
            )
        );
    }
...
sub new {
    ...
    if ($startTime) {
        $song->can('startOffset')
          ...
          : ( $song->{startOffset} = $startTime );
        $args->{'client'}
          ->master->remoteStreamStartTime( Time::HiRes::time() - $startTime );
        $offset = undef;
    }

    main::INFOLOG
      && $log->is_info
      && $log->info( "url: $args->{url} master: $masterUrl offset: ",
        $startTime || 0 );
    ...
    if ( defined($self) ) {
        ...
        ${*$self}{'vars'} = {    # variables which hold state for this instance:
            ...
            'streaming' =>
              1,    # flag for streaming, changes to 0 when all data received
            'fetching' => 0,        # waiting for HTTP data
            'offset'   => $offset
            , # offset for next HTTP request in webm/stream or segment index in dash
        };
    }
...
sub onStop {
    ...
    Plugins::BBCSounds::ActivityManagement::heartBeat( $id,
        Plugins::BBCSounds::ProtocolHandler->getPid( $song->track->url ),
        'paused', floor($elapsed) );
...
sub onStream {
    ...
    Plugins::BBCSounds::ActivityManagement::heartBeat(
        $id,       Plugins::BBCSounds::ProtocolHandler->getPid($url),
        'started', floor( $song->master->controller->playingSongElapsed )
    );
...
sub sysread {
    ...
    if ( my $bytes = min( length $v->{'outBuf'}, $maxBytes ) ) {
        ...
        if ( time() > $nextHeartbeat ) {
            ...
            my $masterUrl = $song->track()->url;
            Plugins::BBCSounds::ActivityManagement::heartBeat(
                Plugins::BBCSounds::ProtocolHandler->getId($masterUrl),
                Plugins::BBCSounds::ProtocolHandler->getPid($masterUrl),
                'heartbeat',
                floor( $song->master->controller->playingSongElapsed )
            );
            $nextHeartbeat = time() + 30;
        }
        $! = EINTR;
...
sub getNextTrack {
    ...
    $song->pluginData( lastpos => ( $masterUrl =~ /&lastpos=([\d]+)/ )[0]
          || 0 );
    $masterUrl =~ s/&.*//;
...
'http://open.live.bbc.co.uk/mediaselector/6/redir/version/2.0/mediaset/audio-syndication-dash/proto/http/vpid/'
    ...
    push @allowDASH,
      (
        [ 'audio_eng=320000',  'aac', 320_000 ],
        [ 'audio_eng=128000',  'aac', 128_000 ],
        [ 'audio_eng_1=96000', 'aac', 96_000 ],
        [ 'audio_eng_1=48000', 'aac', 48_000 ]
      );
    ...
    $session->send_request(
        {
            ...
            onBody  => sub {
                ...
                my $startBase =
                  'http://' . $endURI->host . dirname( $endURI->path ) . '/';
                ...
                my $mpd = XMLin(
                    ...
                    ForceContent => 1,
                    ForceArray =>
                      [ 'AdaptationSet', 'Representation', 'Period' ]
                );
                ...
                foreach my $adaptation (@$adaptationSet) {
                    if ( $adaptation->{'mimeType'} eq 'audio/mp4' ) {
                        ...
                        foreach my $representation (
                            @{ $adaptation->{'Representation'} } )
                        {

                            next
                              unless my ($index) =
                              grep { $$allow[$_][0] eq $representation->{'id'} }
                              ( 0 .. @$allow - 1 );
                            main::INFOLOG
                              && $log->is_info
                              && $log->info(
                                "found matching format $representation->{'id'}"
                              );
                            next
                ...
                my ( $misc, $hour, $min, $sec ) = $duration =~
/P(?:([^T]*))T(?:(\d+)H)?(?:(\d+)M)?(?:([+-]?([0-9]*[.])?[0-9]+)S)?/;
                $duration =
                  ( $sec || 0 ) +
                  ( ( $min  || 0 ) * 60 ) +
                  ( ( $hour || 0 ) * 3600 );
                ...
                my $props = {
                    ...
                    initializeURL =>
                      $selRepres->{'SegmentTemplate'}->{'initialization'}
                    ...
                    channels =>
                      $selRepres->{'AudioChannelConfiguration'}->{'value'}
                ...
                #fix urls
                $props->{initializeURL} =~
                  s/\$RepresentationID\$/$selRepres->{id}/;
                $props->{segmentURL} =~
                  s/\$RepresentationID\$/$selRepres->{id}/;
                $props->{endNumber} = ceil(
                    $duration / (
                        $props->{segmentDuration} / $props->{segmentTimescale}
                    )
                );
